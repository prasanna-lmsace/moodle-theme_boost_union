...
Feature: Configuring the theme_boost_union plugin on the "Smart menus" page, applying different presentation options to the individual smart menu items
  ...
  Scenario Outline: Smartmenus: Menu items: Presentation - Display the menu items title with icon
    ...
    Then I change window size to "mobile"
    And I <bottomtitleshould> see smart menu "Quick links" item "Resources" in location "Bottom"
    And ".icon.fa-info-circle" "css_element" should exist in the ".primary-navigation .dropdown-item" "css_element"
    And ".icon.fa-info-circle" "css_element" should exist in the ".boost-union-menubar .dropdown-item" "css_element"
    And ".icon.fa-info-circle" "css_element" should exist in the "#usermenu-carousel .carousel-item.submenu .dropdown-item" "css_element"
    And ".icon.fa-info-circle" "css_element" should exist in the ".bottom-navigation .dropdown-item" "css_element"
    Then I change window size to "large"

    Examples:
      | presentationtitle                                      | titleshouldornot | bottomtitleshould |
